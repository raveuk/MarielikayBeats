    ...
    fun isVideoAllowed(video: Video): Boolean {
        ...
        when (selectedAgeLevel) {
            AgeRating.FIVE_PLUS -> {
                ...
                val hasKidFriendlyIndicator = KID_FRIENDLY_INDICATORS.any { indicator ->
                    content.contains(indicator)
                }
                return hasKidFriendlyIndicator
            }
            AgeRating.TEN_PLUS -> {
            ...
            AgeRating.TWELVE_PLUS, AgeRating.FOURTEEN_PLUS, AgeRating.SIXTEEN_PLUS -> {
